...
export const validateResponses: Handler = (req, res, next) => {
  ...
  if (typeof response.validateResponse === 'function') {
    ...
    res.send = function expressOpenAPISend(...args) {
      ...
      const validation: Optional<Partial<OpenAPIResponseValidatorValidationError>> =
        response.validateResponse(res.statusCode as any as string, body) as any;
      if (!validation) {
        send.apply(res, args);
        return;
      }
      const validationMessage = `Invalid response for status code ${res.statusCode} for ${
        req.url
